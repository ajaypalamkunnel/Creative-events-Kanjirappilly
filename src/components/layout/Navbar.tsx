...
import { Menu, X, Phone, CalendarHeart, Sparkles, ChevronDown } from "lucide-react";
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative w-11 h-11 rounded-full overflow-hidden border-2 border-brand-gold/60 group-hover:border-brand-gold transition-colors duration-300 shadow-md bg-white">
            <Image
              src="/assets/Logo.jpg"
          ...
          <div>
            ...
            <span className="text-[10px] tracking-[0.18em] uppercase text-brand-gold block font-medium">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {phoneDropdown && (
              <div className="absolute top-full right-0 mt-2 w-56 rounded-xl bg-brand-dark/95 backdrop-blur-xl border border-brand-gold/30 shadow-2xl p-2 z-50">
                <div className="text-[10px] uppercase tracking-wider text-brand-gold font-semibold px-3 py-1">
                  Call Our Team (Kanjirappilly)
                </div>
                {siteConfig.contact.phones.map((p, idx) => (
                  <a
                    key={idx}
                    href={`tel:${p}`}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-slate-200 hover:text-brand-goldLight hover:bg-white/10 transition-colors"
                  >
      ...
      {mobileMenuOpen && (
        <div className="sm:hidden bg-brand-darker/95 backdrop-blur-2xl border-b border-brand-gold/30 px-6 py-6 animate-fadeIn shadow-2xl">
          ...
          <div className="flex flex-col gap-2 pt-2">
            <div className="text-xs uppercase tracking-wider text-brand-gold font-semibold mb-1">
              Contact Numbers:
            </div>
            {siteConfig.contact.phones.map((p, idx) => (
              <a
                key={idx}
                href={`tel:${p}`}
                className="flex items-center gap-2 py-2 px-3 rounded-lg bg-white/5 text-xs font-semibold text-slate-200 border border-white/10"
              >
                <Phone className="w-3.5 h-3.5 text-brand-gold" />
                {p}
              </a>
